...
public class Method2 {
	...
	public static int init(int[] p, int n) {
		int[] r = new int[n];
		for (int i = 0; i < n; i++) {
	...
	/**
	 * 我们可以发现，长度为7时，将其切割为长度4与长度3的钢条，并对两个钢条分别求最优解：长度4的最优解为r[4] = 10（2 +
	...
	public static int cutrod(int[] p, int n, int[] r) {
		...
		for (int i = 1; i <= n; i++) {
			q = CompareUtils.max(q, p[i] + cutrod(p, n - i, r));
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(init(price, 10));
